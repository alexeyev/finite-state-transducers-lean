...
/-! ### Cyclic rotation algebra (conjecture-mined)

    EN: The conjugacy criterion below expresses the second loop output as the *cyclic rotation*
        `β₁.drop|w| ++ β₁.take|w|` of the first.  This block develops that rotation operation in its
        own right.  The laws here were surfaced by the property-based conjecture miner
        (`scripts/conjecture_miner.lean`): it proposed each as a candidate that survived exhaustive
        small-model testing, and -- instructively -- *refuted* the naive "rotation wraps modulo
        length" laws, exposing that `rot` defined via raw `drop`/`take` **saturates** for shift
        amounts `>= |w|` (rotating past the end leaves the word fixed) rather than wrapping.  The
        commutation theorem `rot_comm` therefore holds for a subtler reason than periodicity, and
        its proof splits into the genuine-rotation, wrap-around, and saturating regimes. -/

/-- Cyclic rotation of a word by `k` positions: move the first `k` letters to the back.  For
    `k >= |w|` this saturates to the identity (see `rot_ge_length`). -/
def rot (k : Nat) (w : List α) : List α := w.drop k ++ w.take k

/-- `rot 0` is the identity. -/
@[simp] theorem rot_zero (w : List α) : rot 0 w = w := by simp [rot]

/-- Rotation preserves length. -/
@[simp] theorem rot_length (k : Nat) (w : List α) : (rot k w).length = w.length := by
  simp only [rot, List.length_append, List.length_drop, List.length_take]; omega

/-- Rotating by exactly `|w|` returns the word (saturation boundary). -/
theorem rot_length_self (w : List α) : rot w.length w = w := by simp [rot]

/-- **Saturation:** rotating by `k >= |w|` leaves the word unchanged.  This is the key way `rot`
    departs from textbook cyclic rotation, and the miner's refutations pinned it down. -/
theorem rot_ge_length (k : Nat) (w : List α) (h : w.length ≤ k) : rot k w = w := by
  simp only [rot, List.drop_eq_nil_of_le h, List.take_of_length_le h, List.nil_append]

/-- Choice-free `drop`/`take` interchange (the Mathlib `List.drop_take` pulls in
    `Classical.choice`; this inductive proof stays within the project's axiom budget). -/
private theorem drop_take_cf (i j : Nat) (l : List α) :
  ...
  | cons x xs ih =>
    ...
    | succ j => cases i with
      | zero => simp
      | succ i =>
        simp only [List.take_succ_cons, List.drop_succ_cons, Nat.succ_sub_succ]; exact ih i j

/-- **Cyclic composition, in range.**  When `a + b <= |w|`, rotating by `b` then by `a` is
    rotating by `a + b` -- genuine cyclic behaviour before any saturation. -/
theorem rot_cyclic_in_range (a b : Nat) (w : List α) (hab : a + b ≤ w.length) :
    rot a (rot b w) = rot (a + b) w := by
  unfold rot
  have hdroplen : (w.drop b).length = w.length - b := List.length_drop
  rw [List.drop_append, List.take_append, hdroplen]
  have e1 : a - (w.length - b) = 0 := by omega
  rw [e1]
  simp only [List.drop_zero, List.take_zero, List.append_nil]
  rw [List.drop_drop, List.take_drop, Nat.add_comm b a, List.append_assoc]
  congr 1
  have hbtake : List.take b w = List.take b (List.take (a + b) w) := by
    rw [List.take_take]; congr 1; omega
  rw [hbtake, List.take_append_drop]

/-- **Cyclic composition, wrap-around.**  When both shifts are in range (`a, b <= |w|`) but their
    sum overflows (`|w| <= a + b`), rotating by `b` then `a` is rotating by `a + b - |w|` -- the
    composition wraps once around the word. -/
theorem rot_cyclic_wrap (a b : Nat) (w : List α)
    (ha : a ≤ w.length) (hb : b ≤ w.length) (hsum : w.length ≤ a + b) :
    rot a (rot b w) = rot (a + b - w.length) w := by
  have hD : (w.drop b).length = w.length - b := List.length_drop
  show (w.drop b ++ w.take b).drop a ++ (w.drop b ++ w.take b).take a
     = w.drop (a + b - w.length) ++ w.take (a + b - w.length)
  rw [List.drop_append, List.take_append, hD]
  have hda : a - (w.length - b) = a + b - w.length := by omega
  rw [hda]
  rw [List.drop_eq_nil_of_le (show (w.drop b).length ≤ a by rw [hD]; omega),
      List.take_of_length_le (show (w.drop b).length ≤ a by rw [hD]; omega), List.nil_append]
  have httm : List.take (a + b - w.length) (w.take b) = w.take (a + b - w.length) := by
    rw [List.take_take]; congr 1; omega
  rw [httm, drop_take_cf, ← List.append_assoc]
  congr 1
  have hsplit := List.take_append_drop (b - (a + b - w.length)) (List.drop (a + b - w.length) w)
  rw [show List.drop (b - (a + b - w.length)) (List.drop (a + b - w.length) w) = w.drop b by
        rw [List.drop_drop]; congr 1; omega] at hsplit
  exact hsplit

/-- **Rotations commute:** `rot a (rot b w) = rot b (rot a w)`, unconditionally.  Surfaced by the
    conjecture miner.  The proof is a four-way case split -- both shifts in range with sum in range
    (`rot_cyclic_in_range`) or wrapping (`rot_cyclic_wrap`), or either shift saturating
    (`rot_ge_length`) -- reflecting that `rot` is not a clean Z/|w| action but a saturating one. -/
theorem rot_comm (a b : Nat) (w : List α) : rot a (rot b w) = rot b (rot a w) := by
  rcases Nat.lt_or_ge a w.length with ha | ha
  · rcases Nat.lt_or_ge b w.length with hb | hb
    · rcases Nat.lt_or_ge (a + b) (w.length + 1) with hsum | hsum
      · rw [rot_cyclic_in_range a b w (by omega), rot_cyclic_in_range b a w (by omega), Nat.add_comm]
      · rw [rot_cyclic_wrap a b w (Nat.le_of_lt ha) (Nat.le_of_lt hb) (by omega),
            rot_cyclic_wrap b a w (Nat.le_of_lt hb) (Nat.le_of_lt ha) (by omega), Nat.add_comm]
    · rw [rot_ge_length b w hb, rot_ge_length b (rot a w) (by rw [rot_length]; exact hb)]
  · rw [rot_ge_length a w ha, rot_ge_length a (rot b w) (by rw [rot_length]; exact ha)]
...
/-- **The echo step-function reproduces its input.**  `demoEchoδ` emits each input letter and
    stays put, so its unique run on `x` from any state `q` is `[(x, q)]` — it computes the identity
    on words.  (Surfaced by the property-based conjecture miner; lifts the `#eval` demo below to a
    proved theorem.) -/
theorem rtRuns_demoEchoδ (q : Fin 2) (x : List (Fin 2)) :
    rtRuns demoEchoδ q x = [(x, q)] := by
  induction x generalizing q with
  | nil => rfl
  | cons a x ih =>
      simp only [rtRuns, demoEchoδ, List.flatMap_cons, List.flatMap_nil, List.append_nil,
        ih, List.map_cons, List.map_nil, olist, List.singleton_append]
...
/-- **`structOK` is invariant under swapping the two runs.**  Exchanging `(α₁, β₁)` with
    `(α₂, β₂)` leaves the boolean structural check unchanged: the lag and reverse-lag branches map
    onto each other and each conjugacy equation is symmetric under the swap.  (Discovered by the
    property-based conjecture miner — it survived ~10⁶ random tuples over `Fin 2`/`Fin 3` before
    being proved here.)  Consequence: the per-loop obligation in the twinning criterion is
    order-independent in the two runs, so a decision procedure may inspect each co-reachable state
    pair in a single canonical order. -/
theorem structOK_swap [DecidableEq B] (α₁ α₂ β₁ β₂ : Word B) :
    structOK α₁ α₂ β₁ β₂ = structOK α₂ α₁ β₂ β₁ := by
  unfold structOK
  by_cases h12 : α₁ <+: α₂
  · by_cases h21 : α₂ <+: α₁
    · have heq : α₁ = α₂ := prefix_antisymm α₁ α₂ h12 h21
      subst heq
      simp only [if_pos h12, List.drop_length, List.append_nil, List.nil_append]
      by_cases hb : β₁ = β₂
      · rw [hb]
      · rw [decide_eq_false (by simpa using hb), decide_eq_false (by simpa using fun h => hb h.symm)]
    · simp only [if_pos h12, if_neg h21]
  · by_cases h21 : α₂ <+: α₁
    · simp only [if_neg h12, if_pos h21]
    · simp only [if_neg h12, if_neg h21]
      by_cases hd : β₁ = [] ∧ β₂ = []
      · rw [decide_eq_true hd, decide_eq_true ⟨hd.2, hd.1⟩]
      · rw [decide_eq_false (by simpa using hd),
            decide_eq_false (by simp only [not_and]; intro h2 h1; exact (not_and.mp hd) h1 h2)]

/-- **`structOK` accepts the diagonal.**  A loop compared against *itself* — equal reaching
    outputs, equal loop outputs — always passes.  (Also surfaced by the conjecture miner.) -/
theorem structOK_self [DecidableEq B] (α β : Word B) :
    structOK α α β β = true := by
  unfold structOK
  simp only [if_pos (List.prefix_refl α), List.drop_length, List.append_nil, List.nil_append,
    decide_eq_true_eq]

/-- **The structural check is order-independent in the two runs.**  If every co-reachable loop
    passes `structOK` with the runs in one order, the same holds with the runs swapped — a direct
    lift of `structOK_swap`.  This is what licenses checking each unordered state pair once. -/
theorem twinning_structOK_symm [DecidableEq B] {T : Transducer A B σ}
    (hp : ∀ (p₁ p₂ : σ) (u : Word A) (α₁ α₂ : Word B) (v : Word A) (β₁ β₂ : Word B),
       Reaches T u α₁ p₁ → Reaches T u α₂ p₂ →
  ...
  intro p₁ p₂ u α₁ α₂ v β₁ β₂ hr1 hr2 hl1 hl2
  rw [← structOK_swap]
  exact hp p₁ p₂ u α₁ α₂ v β₁ β₂ hr1 hr2 hl1 hl2
...
/-! ## Basic transducers and the letter-to-letter (Mealy) subclass

    EN: Two foundational pieces that round out the model.  First, the **identity transducer**
        `idT`: a one-state machine that copies its input to its output, realizing the diagonal
        relation `{(x, x)}` (`idT_realizes`).  Second, the **Mealy machine** -- the
        *letter-to-letter* (length-preserving) deterministic transducer that reads one input
        letter and emits exactly one output letter per step, with no ε-moves.  Mealy machines form
        an important restricted class with cleaner theory: every realized pair satisfies
        `|y| = |x|` (`Mealy.realizes_length`) and the realized relation is a genuine function
        (`Mealy.realizes_functional`).  `Mealy.toTransducer` embeds the class into the general
        relational framework, with `Mealy.toTransducer_realizes` proving the two semantics agree;
        `idMealy` exhibits the identity as a Mealy machine. -/

/-- The **identity transducer** over `A`: one state, copies each input letter to the output,
    accepts immediately.  Realizes the diagonal relation. -/
def idT (A : Type) : Transducer A A Unit where
  start := fun _ => True
  accept := fun _ => True
  step := fun _ oa ob _ => oa = ob ∧ oa.isSome

private theorem idT_path_diag (n : Nat) (p q : Unit) (x y : Word A) :
    PathN (idT A) n p x y q → x = y := by
  induction n generalizing p q x y with
  | zero => intro h; obtain ⟨_, hx, hy⟩ := PathN.zero_inv h; rw [hx, hy]
  | succ m ih =>
      intro h
      obtain ⟨oa, ob, r, x', y', hstep, htail, hx, hy⟩ := PathN.step_inv h
      obtain ⟨hoab, _⟩ := hstep
      rw [hx, hy, hoab, ih r q x' y' htail]

private theorem idT_diag_path (x : Word A) : PathN (idT A) x.length () x x () := by
  induction x with
  | nil => exact PathN.nil ()
  | cons a x ih =>
      have hstep : (idT A).step () (some a) (some a) () := ⟨rfl, rfl⟩
      have := PathN.step () (some a) (some a) () x.length x x () hstep ih
      simpa [olist] using this

/-- **The identity transducer realizes the diagonal:** `realizes (idT A) x y ↔ x = y`. -/
theorem idT_realizes (x y : Word A) : realizes (idT A) x y ↔ x = y := by
  constructor
  · rintro ⟨p, q, _, _, n, hpath⟩; exact idT_path_diag n p q x y hpath
  · rintro rfl; exact ⟨(), (), trivial, trivial, x.length, idT_diag_path x⟩

/-! ### Rational relations form a monoid under composition

    EN: The closure theorems (`rational_compose`, `rational_atom`, `rational_zero`, …) give the
        building blocks; this block records the *algebraic structure* they sit in.  `Rcomp` names
        relational composition `(R ⋄ S) x z = ∃ y, R x y ∧ S y z` -- the operation realized by
        transducer `compose` -- and `Rcomp_assoc` proves it associative.  `Rid` is the full diagonal
        (identity) relation on words, a two-sided unit (`Rid_comp_left`/`right`) that is itself
        rational (`Rid_rational`, witnessed by `idT`).  So `(WordRel A A, Rcomp, Rid)` is a monoid,
        and rationality is closed under the operation and contains the unit: the rational relations
        on a fixed alphabet form a *submonoid*. -/

/-- **Relational composition**: `(R ⋄ S) x z` holds when some intermediate `y` has `R x y` and
    `S y z`.  This is the relation realized by transducer `compose` (`realizes_compose`). -/
def Rcomp (R : WordRel A B) (S : WordRel B C) : WordRel A C := fun x z => ∃ y, R x y ∧ S y z

/-- **Relational composition is associative.** -/
theorem Rcomp_assoc (R : WordRel A B) (S : WordRel B C) (U : WordRel C D) :
    Rcomp (Rcomp R S) U = Rcomp R (Rcomp S U) := by
  funext x z; apply propext
  constructor
  · rintro ⟨w, ⟨y, hxy, hyw⟩, hwz⟩; exact ⟨y, hxy, w, hyw, hwz⟩
  · rintro ⟨y, hxy, w, hyw, hwz⟩; exact ⟨w, ⟨y, hxy, hyw⟩, hwz⟩

/-- **Rationality is closed under relational composition** (operator form of `rational_compose`). -/
theorem Rcomp_rational {R : WordRel A B} {S : WordRel B C}
    (hR : Rational R) (hS : Rational S) : Rational (Rcomp R S) :=
  rational_compose hR hS

/-- The **diagonal (identity) relation** on words over `A`: each word relates to itself only. -/
def Rid (A : Type) : WordRel A A := fun x y => x = y

/-- The diagonal is a **left unit** for relational composition. -/
theorem Rid_comp_left (R : WordRel A B) : Rcomp (Rid A) R = R := by
  funext x y; apply propext
  constructor
  · rintro ⟨z, hxz, hzy⟩; rw [Rid] at hxz; rw [hxz]; exact hzy
  · intro h; exact ⟨x, rfl, h⟩

/-- The diagonal is a **right unit** for relational composition. -/
theorem Rid_comp_right (R : WordRel A B) : Rcomp R (Rid B) = R := by
  funext x y; apply propext
  constructor
  · rintro ⟨z, hxz, hzy⟩; rw [Rid] at hzy; rw [← hzy]; exact hxz
  · intro h; exact ⟨y, h, rfl⟩

/-- **The diagonal relation is rational** -- realized by the identity transducer `idT`. -/
theorem Rid_rational : Rational (Rid A) :=
  ⟨Unit, idT A, fun x y => by rw [idT_realizes]; rfl⟩

/-! ### The idempotent semiring of word relations

    EN: Composition gives word relations a monoid structure (previous block); union gives a second
        one, and the two interact as a *semiring*.  Union (`RUnion`) is associative, commutative,
        and idempotent with the empty relation `Rzero` as unit; composition `Rcomp` distributes over
        union on both sides; and `Rzero` absorbs under composition (`Rzero ⋄ R = Rzero`).  So
        `(WordRel A A, RUnion, Rcomp, Rzero, Rid)` is an **idempotent semiring** -- the algebraic
        home of rational relations -- and since rationality is closed under both operations
        (`rational_union`, `Rcomp_rational`) and contains both constants (`Rzero_rational`,
        `Rid_rational`), the rational relations form a *subsemiring*. -/

/-- The **empty relation**, named to pair with `RUnion` as the additive unit. -/
def Rzero (A B : Type) : WordRel A B := fun _ _ => False

/-- **Union is associative.** -/
theorem RUnion_assoc (R S T : WordRel A B) :
    RUnion (RUnion R S) T = RUnion R (RUnion S T) := by
  funext x y; apply propext; unfold RUnion
  constructor
  · rintro ((h | h) | h)
    · exact Or.inl h
...
theorem RUnion_comm (R S : WordRel A B) : RUnion R S = RUnion S R := by
  funext x y; apply propext; unfold RUnion
  exact ⟨fun h => h.symm, fun h => h.symm⟩

/-- **Union is idempotent** -- the feature distinguishing this from a ring (no additive inverses). -/
theorem RUnion_idem (R : WordRel A B) : RUnion R R = R := by
  funext x y; apply propext; unfold RUnion
  exact ⟨fun h => h.elim id id, fun h => Or.inl h⟩

/-- The empty relation is a **left unit** for union. -/
theorem Rzero_union_left (R : WordRel A B) : RUnion (Rzero A B) R = R := by
  funext x y; apply propext; unfold RUnion Rzero
  exact ⟨fun h => h.elim False.elim id, fun h => Or.inr h⟩

/-- The empty relation is a **right unit** for union. -/
theorem Rzero_union_right (R : WordRel A B) : RUnion R (Rzero A B) = R := by
  funext x y; apply propext; unfold RUnion Rzero
  exact ⟨fun h => h.elim id False.elim, fun h => Or.inl h⟩

/-- **The empty relation is rational** (named form of `rational_zero`). -/
theorem Rzero_rational : Rational (Rzero A B) := rational_zero

/-- **Composition distributes over union on the left:** `R ⋄ (S ∪ T) = (R ⋄ S) ∪ (R ⋄ T)`. -/
theorem Rcomp_union_left (R : WordRel A B) (S T : WordRel B C) :
    Rcomp R (RUnion S T) = RUnion (Rcomp R S) (Rcomp R T) := by
  funext x z; apply propext; unfold Rcomp RUnion
  constructor
  · rintro ⟨y, hxy, (hS | hT)⟩
    · exact Or.inl ⟨y, hxy, hS⟩
    · exact Or.inr ⟨y, hxy, hT⟩
  · rintro (⟨y, hxy, hS⟩ | ⟨y, hxy, hT⟩)
    · exact ⟨y, hxy, Or.inl hS⟩
    · exact ⟨y, hxy, Or.inr hT⟩

/-- **Composition distributes over union on the right:** `(R ∪ S) ⋄ T = (R ⋄ T) ∪ (S ⋄ T)`. -/
theorem Rcomp_union_right (R S : WordRel A B) (T : WordRel B C) :
    Rcomp (RUnion R S) T = RUnion (Rcomp R T) (Rcomp S T) := by
  funext x z; apply propext; unfold Rcomp RUnion
  constructor
  · rintro ⟨y, (hR | hS), hyz⟩
    · exact Or.inl ⟨y, hR, hyz⟩
    · exact Or.inr ⟨y, hS, hyz⟩
  · rintro (⟨y, hR, hyz⟩ | ⟨y, hS, hyz⟩)
    · exact ⟨y, Or.inl hR, hyz⟩
    · exact ⟨y, Or.inr hS, hyz⟩

/-- The empty relation **absorbs on the left** under composition: `Rzero ⋄ R = Rzero`. -/
theorem Rzero_comp_left (R : WordRel B C) : Rcomp (Rzero A B) R = Rzero A C := by
  funext x z; apply propext; unfold Rcomp Rzero
  constructor
  · rintro ⟨y, hxy, _⟩; exact hxy
  · intro h; exact h.elim

/-- The empty relation **absorbs on the right** under composition: `R ⋄ Rzero = Rzero`. -/
theorem Rzero_comp_right (R : WordRel A B) : Rcomp R (Rzero B C) = Rzero A C := by
  funext x z; apply propext; unfold Rcomp Rzero
  constructor
  · rintro ⟨y, _, hyz⟩; exact hyz
  · intro h; exact h.elim

/-! ### Kleene algebra: the composition star

    EN: The semiring above gains a *star* operation, making the relations a **Kleene algebra**.
        `Rstar R` is the reflexive-transitive closure of an endorelation `R` under composition
        (`R* = ⋃ₙ Rⁿ`), defined inductively.  It satisfies the four Kleene-algebra axioms: the two
        unfolding laws `R* = Rid ∪ R⋄R* = Rid ∪ R*⋄R` (`Rstar_unfold_left`/`right`), and the two
        induction laws making `R*` the *least* fixpoint -- `R⋄S ⊆ S → R*⋄S ⊆ S` and its dual
        (`Rstar_induction_left`/`right`).  Also: monotonicity, `R ⊆ R*`, transitivity, and
        idempotence `(R*)* = R*`.

        IMPORTANT (an honest scope boundary): this star is the closure operation on *relations*, and
        the axioms hold for all word relations.  But unlike union and composition, **the rational
        relations are NOT closed under the composition star** -- `Rational R` does not imply
        `Rational (Rstar R)`.  The infinite iteration `⋃ₙ Rⁿ` can escape the rational relations
        (rational relations are closed under *finite* composition, not under composition-closure).
        So there is a Kleene algebra of word relations, but the rational relations form a
        *subsemiring* (previous blocks), not a Kleene subalgebra.  No rationality-preservation
        theorem is stated for `Rstar`, by design -- it would be false.  (Note this is the
        composition star, distinct from the concatenation star of `realizes_star`, which IS a
        rationality-preserving operation on transductions.) -/

/-- **Composition star** `R*`: the reflexive-transitive closure of an endorelation `R` under
    relational composition.  `R* x z` holds when `z` is reachable from `x` by finitely many
    `R`-steps. -/
inductive Rstar (R : WordRel A A) : WordRel A A
  | refl (x : Word A) : Rstar R x x
  | tail {x y z : Word A} : Rstar R x y → R y z → Rstar R x z

/-- **Relational inclusion** `R ⊆ S`. -/
def Rsub (R S : WordRel A A) : Prop := ∀ x y, R x y → S x y

/-- **Transitivity of the star** (closure under composition with itself). -/
theorem Rstar_trans (R : WordRel A A) {x y z : Word A}
    (h1 : Rstar R x y) (h2 : Rstar R y z) : Rstar R x z := by
  induction h2 with
  | refl w => exact h1
  | @tail a b c hab hbc ih => exact Rstar.tail (ih h1) hbc

/-- **Prepend a step:** `R x y → R* y z → R* x z` (head form, dual to the `tail` constructor). -/
theorem Rstar_head (R : WordRel A A) {x y z : Word A}
    (hxy : R x y) (hyz : Rstar R y z) : Rstar R x z :=
  Rstar_trans R (Rstar.tail (Rstar.refl x) hxy) hyz

/-- **`R ⊆ R*`:** the star contains the base relation. -/
theorem R_le_Rstar (R : WordRel A A) : Rsub R (Rstar R) :=
  fun x _y h => Rstar.tail (Rstar.refl x) h

/-- **Head decomposition.**  A star path is reflexive or factors as a first `R`-step then a star. -/
theorem Rstar_cases_head (R : WordRel A A) {x z : Word A} (h : Rstar R x z) :
    x = z ∨ ∃ y, R x y ∧ Rstar R y z := by
  induction h with
  | refl => exact Or.inl rfl
  | @tail a b c hab hbc ih =>
      cases ih with
      | inl heq => exact Or.inr ⟨c, heq ▸ hbc, Rstar.refl c⟩
      | inr h => obtain ⟨y, hay, hyb⟩ := h; exact Or.inr ⟨y, hay, Rstar.tail hyb hbc⟩

/-- **Left unfolding (Kleene):** `R* = Rid ∪ R ⋄ R*`. -/
theorem Rstar_unfold_left (R : WordRel A A) :
    Rstar R = RUnion (Rid A) (Rcomp R (Rstar R)) := by
  funext x z; apply propext
  constructor
  · intro h
    cases Rstar_cases_head R h with
    | inl heq => exact Or.inl heq
    | inr h => obtain ⟨y, hxy, hyz⟩ := h; exact Or.inr ⟨y, hxy, hyz⟩
  · rintro (h | ⟨y, hxy, hyz⟩)
    · rw [Rid] at h; rw [h]; exact Rstar.refl z
    · exact Rstar_head R hxy hyz

/-- **Right unfolding (Kleene):** `R* = Rid ∪ R* ⋄ R`. -/
theorem Rstar_unfold_right (R : WordRel A A) :
    Rstar R = RUnion (Rid A) (Rcomp (Rstar R) R) := by
  funext x z; apply propext
  constructor
  · intro h
    cases h with
    | refl => exact Or.inl rfl
    | tail hxy hyz => exact Or.inr ⟨_, hxy, hyz⟩
  · rintro (h | ⟨y, hxy, hyz⟩)
    · rw [Rid] at h; rw [h]; exact Rstar.refl z
    · exact Rstar.tail hxy hyz

/-- Absorption helper for left induction: if `R⋄S ⊆ S`, a star path absorbs into `S`. -/
theorem Rstar_absorb_left (R S : WordRel A A) (h : Rsub (Rcomp R S) S)
    {x y : Word A} (hxy : Rstar R x y) : ∀ z, S y z → S x z := by
  induction hxy with
  | refl => intro z hz; exact hz
  | @tail a b c hab hbc ih =>
      intro z hz
      have hbz : S b z := h b z ⟨c, hbc, hz⟩
      exact ih z hbz

/-- **Left induction (Kleene, least fixpoint):** `R ⋄ S ⊆ S → R* ⋄ S ⊆ S`.  This makes `R*` the
    *least* reflexive-transitive relation containing `R`. -/
theorem Rstar_induction_left (R S : WordRel A A) (h : Rsub (Rcomp R S) S) :
    Rsub (Rcomp (Rstar R) S) S := by
  rintro x z ⟨y, hxy, hyz⟩
  exact Rstar_absorb_left R S h hxy z hyz

/-- Absorption helper for right induction. -/
theorem Rstar_absorb_right (R S : WordRel A A) (h : Rsub (Rcomp S R) S)
    {y z : Word A} (hyz : Rstar R y z) : ∀ x, S x y → S x z := by
  induction hyz with
  | refl => intro x hx; exact hx
  | @tail a b c hab hbc ih =>
      intro x hx
      have hxb : S x b := ih x hx
      exact h x c ⟨b, hxb, hbc⟩

/-- **Right induction (Kleene, least fixpoint):** `S ⋄ R ⊆ S → S ⋄ R* ⊆ S`. -/
theorem Rstar_induction_right (R S : WordRel A A) (h : Rsub (Rcomp S R) S) :
    Rsub (Rcomp S (Rstar R)) S := by
  rintro x z ⟨y, hxy, hyz⟩
  exact Rstar_absorb_right R S h hyz x hxy

/-- **Monotonicity:** `R ⊆ S → R* ⊆ S*`. -/
theorem Rstar_mono (R S : WordRel A A) (h : Rsub R S) : Rsub (Rstar R) (Rstar S) := by
  intro x z hxz
  induction hxz with
  | refl => exact Rstar.refl _
  | @tail a b c hab hbc ih => exact Rstar.tail ih (h b c hbc)

/-- **Idempotence:** `(R*)* = R*`. -/
theorem Rstar_idem (R : WordRel A A) : Rstar (Rstar R) = Rstar R := by
  funext x z; apply propext
  constructor
  · intro h
    induction h with
    | refl => exact Rstar.refl _
    | @tail a b c hab hbc ih => exact Rstar_trans R ih hbc
  · intro h; exact R_le_Rstar (Rstar R) x z h

/-- A **Mealy machine** (letter-to-letter / length-preserving transducer): from each state,
    reading one input letter deterministically emits one output letter and moves on.  Total
    transition and output functions, no ε-moves. -/
structure Mealy (A B σ : Type) where
  start : σ
  out : σ → A → B
  tr  : σ → A → σ
...
/-- Run a Mealy machine on a word, producing the output word and final state. -/
def run (M : Mealy A B σ) : σ → Word A → Word B × σ
  | s, [] => ([], s)
  | s, a :: x => let (y, s') := run M (M.tr s a) x; (M.out s a :: y, s')

/-- The output of `M.run` has the same length as the input -- letter-to-letter. -/
theorem run_length_preserving (M : Mealy A B σ) (s : σ) (x : Word A) :
    (M.run s x).1.length = x.length := by
  induction x generalizing s with
  | nil => rfl
  | cons a x ih => simp only [run, List.length_cons]; rw [ih (M.tr s a)]

/-- The relation realized by a Mealy machine: running from the start state yields `y`. -/
def realizes (M : Mealy A B σ) (x : Word A) (y : Word B) : Prop := (M.run M.start x).1 = y

/-- **Mealy machines are length-preserving:** every realized pair has `|y| = |x|`. -/
theorem realizes_length (M : Mealy A B σ) (x : Word A) (y : Word B) :
    M.realizes x y → y.length = x.length := by
  intro h; rw [← h]; exact run_length_preserving M M.start x

/-- **A Mealy machine is a function:** each input determines its output. -/
theorem realizes_functional (M : Mealy A B σ) (x : Word A) (y₁ y₂ : Word B) :
    M.realizes x y₁ → M.realizes x y₂ → y₁ = y₂ := by
  intro h1 h2; rw [← h1, ← h2]

/-- Embed a Mealy machine as a relational `Transducer`: one input letter, one output letter per
    step, every state accepting. -/
def toTransducer (M : Mealy A B σ) : Transducer A B σ where
  start := fun s => s = M.start
  accept := fun _ => True
  step := fun s oa ob s' => ∃ a, oa = some a ∧ ob = some (M.out s a) ∧ s' = M.tr s a

private theorem run_toPath (M : Mealy A B σ) (s : σ) (x : Word A) :
    PathN M.toTransducer x.length s x (M.run s x).1 (M.run s x).2 := by
  induction x generalizing s with
  | nil => exact PathN.nil s
  | cons a x ih =>
      have hstep : M.toTransducer.step s (some a) (some (M.out s a)) (M.tr s a) := ⟨a, rfl, rfl, rfl⟩
      have hp := PathN.step s (some a) (some (M.out s a)) (M.tr s a) x.length x
        (M.run (M.tr s a) x).1 (M.run (M.tr s a) x).2 hstep (ih (M.tr s a))
      simpa only [run, olist] using hp

private theorem path_toRun (M : Mealy A B σ) (n : Nat) (s s' : σ) (x : Word A) (y : Word B) :
    PathN M.toTransducer n s x y s' → (M.run s x).1 = y ∧ (M.run s x).2 = s' := by
  induction n generalizing s x y with
  | zero => intro h; obtain ⟨hss, hx, hy⟩ := PathN.zero_inv h; subst hx; subst hy; exact ⟨rfl, hss⟩
  | succ m ih =>
      intro h
      obtain ⟨oa, ob, r, x', y', hstep, htail, hx, hy⟩ := PathN.step_inv h
      obtain ⟨a, hoa, hob, hr⟩ := hstep
      subst hoa; subst hob; subst hr; subst hx; subst hy
      obtain ⟨hy', hs'⟩ := ih (M.tr s a) x' y' htail
      simp only [run, olist, List.singleton_append]
      exact ⟨by rw [hy'], hs'⟩

/-- **The embedding preserves the realized relation:** Mealy and transducer semantics agree. -/
theorem toTransducer_realizes (M : Mealy A B σ) (x : Word A) (y : Word B) :
    FST.realizes M.toTransducer x y ↔ M.realizes x y := by
  constructor
  · rintro ⟨p, q, hp, _, n, hpath⟩; subst hp; exact (path_toRun M n M.start q x y hpath).1
  · intro h
    refine ⟨M.start, (M.run M.start x).2, rfl, trivial, x.length, ?_⟩
    have := run_toPath M M.start x; rwa [h] at this


/-- **Composition of Mealy machines.**  Running `M₁ : A→B` then `M₂ : B→C` in lockstep: each input
    letter drives `M₁` to emit a `B`-letter, which immediately drives `M₂`.  The product state is
    `σ₁ × σ₂` and the result is again letter-to-letter, witnessing that the Mealy class is closed
    under composition. -/
def comp (M₁ : Mealy A B σ₁) (M₂ : Mealy B C σ₂) : Mealy A C (σ₁ × σ₂) where
  start := (M₁.start, M₂.start)
  out := fun s a => M₂.out s.2 (M₁.out s.1 a)
  tr := fun s a => (M₁.tr s.1 a, M₂.tr s.2 (M₁.out s.1 a))

/-- The composite run emits, letter by letter, `M₂` applied to `M₁`'s output, state-componentwise. -/
theorem comp_run (M₁ : Mealy A B σ₁) (M₂ : Mealy B C σ₂) (s₁ : σ₁) (s₂ : σ₂) (x : Word A) :
    ((comp M₁ M₂).run (s₁, s₂) x).1 = (M₂.run s₂ (M₁.run s₁ x).1).1 := by
  induction x generalizing s₁ s₂ with
  | nil => rfl
  | cons a x ih =>
      have hL : ((comp M₁ M₂).run (s₁, s₂) (a :: x)).1
              = M₂.out s₂ (M₁.out s₁ a) :: ((comp M₁ M₂).run (M₁.tr s₁ a, M₂.tr s₂ (M₁.out s₁ a)) x).1 := rfl
      have hR : (M₂.run s₂ (M₁.run s₁ (a :: x)).1).1
              = M₂.out s₂ (M₁.out s₁ a) :: (M₂.run (M₂.tr s₂ (M₁.out s₁ a)) (M₁.run (M₁.tr s₁ a) x).1).1 := rfl
      rw [hL, hR, ih (M₁.tr s₁ a) (M₂.tr s₂ (M₁.out s₁ a))]

/-- The composite run from the start states splits as `M₂ ∘ M₁` on outputs. -/
theorem comp_run_split (M₁ : Mealy A B σ₁) (M₂ : Mealy B C σ₂) (x : Word A) :
    ((comp M₁ M₂).run (comp M₁ M₂).start x).1 = (M₂.run M₂.start (M₁.run M₁.start x).1).1 :=
  comp_run M₁ M₂ M₁.start M₂.start x

/-- **Mealy machines are closed under composition:** the composite realizes the relational
    composition of the two transductions, `(M₁ ⨾ M₂)(x) = z ↔ ∃ y, M₁(x) = y ∧ M₂(y) = z`. -/
theorem comp_realizes (M₁ : Mealy A B σ₁) (M₂ : Mealy B C σ₂) (x : Word A) (z : Word C) :
    (comp M₁ M₂).realizes x z ↔ ∃ y, M₁.realizes x y ∧ M₂.realizes y z := by
  constructor
  · intro h
    refine ⟨(M₁.run M₁.start x).1, rfl, ?_⟩
    show (M₂.run M₂.start (M₁.run M₁.start x).1).1 = z
    rw [← comp_run_split]; exact h
  · rintro ⟨y, hy, hz⟩
    show ((comp M₁ M₂).run (comp M₁ M₂).start x).1 = z
    rw [comp_run_split, show (M₁.run M₁.start x).1 = y from hy]; exact hz

/-! ### Causality, behavioural equivalence, and minimization (Mealy)

    EN: This block develops the structural theory that makes Mealy machines *sequential*
        (online) functions and sets up their minimization.  `run_append` is the monoid-action law
        -- running on a concatenation threads the state through, splitting the output -- and from it
        `run_prefix`/`realizes_prefix` give the **causality** property: the output on `x` is a
        prefix of the output on any extension, so a Mealy machine commits to each output letter
        immediately and never revises it.  `StateEquiv` is the Myhill-Nerode equivalence (two
        states agree on every input); `StateEquiv.step`/`of_step` characterize it *coinductively*
        as the greatest bisimulation (equal output letters + equivalent successors), the local
        condition that drives partition-refinement minimization.  `withStart_equiv_realizes` is the
        soundness payoff: rebasing a machine at an equivalent start state preserves its function. -/

/-- **Monoid-action law for `run`.**  Running on a concatenation `x ++ x'` runs on `x`, then runs
    on `x'` from the resulting state, concatenating the outputs and threading the final state. -/
theorem run_append (M : Mealy A B σ) (s : σ) (x x' : Word A) :
    (M.run s (x ++ x')) = ((M.run s x).1 ++ (M.run (M.run s x).2 x').1,
                            (M.run (M.run s x).2 x').2) := by
  induction x generalizing s with
  | nil => rfl
  | cons a x ih => have key := ih (M.tr s a); simp only [Mealy.run, List.cons_append, key]

/-- The output-word component of `run` on a concatenation. -/
theorem run_append_fst (M : Mealy A B σ) (s : σ) (x x' : Word A) :
    (M.run s (x ++ x')).1 = (M.run s x).1 ++ (M.run (M.run s x).2 x').1 := by rw [run_append]

/-- **Causality / online property.**  The output `M` produces on `x` is a *prefix* of what it
    produces on any extension `x ++ x'`.  Each output letter is emitted immediately and never
    revised -- the hallmark of a sequential transducer. -/
theorem run_prefix (M : Mealy A B σ) (s : σ) (x x' : Word A) :
    (M.run s x).1 <+: (M.run s (x ++ x')).1 := by
  rw [run_append_fst]; exact ⟨(M.run (M.run s x).2 x').1, rfl⟩

/-- Realizes-level causality: `M(x) = y` and `M(x ++ x') = w` force `y <+: w`. -/
theorem realizes_prefix (M : Mealy A B σ) (x x' : Word A) (y w : Word B)
    (hy : M.realizes x y) (hw : M.realizes (x ++ x') w) : y <+: w := by
  rw [← hy, ← hw]; exact run_prefix M M.start x x'

/-- **Behavioural (state) equivalence.**  Two states are equivalent when, starting from each, the
    machine emits the same output on *every* input.  This is the Myhill-Nerode equivalence for
    Mealy machines and the basis for minimization. -/
def StateEquiv (M : Mealy A B σ) (s t : σ) : Prop :=
  ∀ x : Word A, (M.run s x).1 = (M.run t x).1

/-- State equivalence is reflexive. -/
theorem StateEquiv.refl (M : Mealy A B σ) (s : σ) : StateEquiv M s s := fun _ => rfl
/-- State equivalence is symmetric. -/
theorem StateEquiv.symm (M : Mealy A B σ) {s t : σ} (h : StateEquiv M s t) : StateEquiv M t s :=
  fun x => (h x).symm
/-- State equivalence is transitive. -/
theorem StateEquiv.trans (M : Mealy A B σ) {s t u : σ}
    (h1 : StateEquiv M s t) (h2 : StateEquiv M t u) : StateEquiv M s u :=
  fun x => (h1 x).trans (h2 x)

/-- **Bisimulation step (forward).**  Equivalent states emit the *same* output letter on every
    input letter and step to equivalent successors -- the local condition of partition refinement. -/
theorem StateEquiv.step (M : Mealy A B σ) {s t : σ} (h : StateEquiv M s t) (a : A) :
    M.out s a = M.out t a ∧ StateEquiv M (M.tr s a) (M.tr t a) := by
  refine ⟨?_, ?_⟩
  · have := h [a]
    simpa only [Mealy.run] using congrArg (fun l => l.headD (M.out s a)) this
  · intro x
    have hax := h (a :: x)
    have e1 : (M.run s (a :: x)).1 = M.out s a :: (M.run (M.tr s a) x).1 := rfl
    have e2 : (M.run t (a :: x)).1 = M.out t a :: (M.run (M.tr t a) x).1 := rfl
    rw [e1, e2] at hax
    exact (List.cons.inj hax).2

/-- **Bisimulation step (converse).**  Equal output letters and equivalent successors on every
    letter imply equivalence.  With `StateEquiv.step`, equivalence is the greatest bisimulation. -/
theorem StateEquiv.of_step (M : Mealy A B σ) {s t : σ}
    (h : ∀ a : A, M.out s a = M.out t a ∧ StateEquiv M (M.tr s a) (M.tr t a)) :
    StateEquiv M s t := by
  intro x
  cases x with
  | nil => rfl
  | cons a x =>
      obtain ⟨hout, hnext⟩ := h a
      have e1 : (M.run s (a :: x)).1 = M.out s a :: (M.run (M.tr s a) x).1 := rfl
      have e2 : (M.run t (a :: x)).1 = M.out t a :: (M.run (M.tr t a) x).1 := rfl
      rw [e1, e2, hout, hnext x]

/-- Rebase a Mealy machine at a new start state, keeping the transition and output functions. -/
def withStart (M : Mealy A B σ) (s : σ) : Mealy A B σ := { M with start := s }

/-- `run` ignores the `start` field: `withStart M s` runs exactly like `M` from any state. -/
theorem withStart_run (M : Mealy A B σ) (s : σ) (q : σ) (x : Word A) :
    (withStart M s).run q x = M.run q x := by
  induction x generalizing q with
  | nil => rfl
  | cons a x ih =>
      show ((withStart M s).out q a :: ((withStart M s).run ((withStart M s).tr q a) x).1,
            ((withStart M s).run ((withStart M s).tr q a) x).2)
         = (M.out q a :: (M.run (M.tr q a) x).1, (M.run (M.tr q a) x).2)
      rw [ih ((withStart M s).tr q a)]; rfl

/-- The relation realized by `withStart M s` is `M` run from `s`. -/
theorem withStart_realizes (M : Mealy A B σ) (s : σ) (x : Word A) (y : Word B) :
    (withStart M s).realizes x y ↔ (M.run s x).1 = y := by
  show ((withStart M s).run (withStart M s).start x).1 = y ↔ (M.run s x).1 = y
  rw [withStart_run]; rfl

/-- **Minimization soundness (start level).**  Rebasing at a behaviourally equivalent start state
    leaves the realized relation unchanged -- replacing a machine by a minimized equivalent is
    sound. -/
theorem withStart_equiv_realizes (M : Mealy A B σ) (s t : σ) (h : StateEquiv M s t)
    (x : Word A) (y : Word B) :
    (withStart M s).realizes x y ↔ (withStart M t).realizes x y := by
  rw [withStart_realizes, withStart_realizes, h x]

/-! ### The minimal (Nerode) machine

    EN: The capstone of the Mealy theory: an explicit minimal machine and its universal property,
        built from the behavioural equivalence above.  The construction is the Nerode one made
        concrete.  `beh` maps each state to the *word function* it computes, and
        `StateEquiv_iff_beh` shows behavioural equivalence is exactly equality of these functions
        (via `funext`).  The minimal machine `minimal M` then takes the **function space itself** as
        its state space: its dynamics (`headOut`, `residual`) are defined *intrinsically* on word
        functions, with no reference to `M`'s states, so behaviourally-equal states of `M` collapse
        to one.  `minimal_realizes` proves it computes the same relation as `M`; `minimal_reduced`
        proves it is genuinely minimal -- distinct reachable states have distinct behaviours, i.e.
        the machine carries no redundancy. -/

/-- The **behaviour** of a state: the word function it computes (its output on each input). -/
def beh (M : Mealy A B σ) (s : σ) : Word A → Word B := fun x => (M.run s x).1

/-- **State equivalence is equality of behaviours** (the Nerode characterization, via `funext`). -/
theorem StateEquiv_iff_beh (M : Mealy A B σ) (s t : σ) :
    StateEquiv M s t ↔ beh M s = beh M t := by
  constructor
  · intro h; exact funext h
  · intro h x; exact congrFun h x

/-- Behaviour on the empty word is empty. -/
theorem beh_nil (M : Mealy A B σ) (s : σ) : beh M s [] = [] := rfl

/-- **Behaviour recursion.**  The behaviour at `s` on `a :: x` is `M.out s a` followed by the
    behaviour at the successor `M.tr s a` on `x` -- behaviours themselves evolve like a machine. -/
theorem beh_cons (M : Mealy A B σ) (s : σ) (a : A) (x : Word A) :
    beh M s (a :: x) = M.out s a :: beh M (M.tr s a) x := rfl

/-- The behaviour transition is well-defined: equal behaviours have equal successor behaviours. -/
theorem beh_tr_congr (M : Mealy A B σ) (s t : σ) (a : A) (h : beh M s = beh M t) :
    beh M (M.tr s a) = beh M (M.tr t a) := by
  apply funext; intro x
  have := congrFun h (a :: x)
  rw [beh_cons, beh_cons] at this
  exact (List.cons.inj this).2

/-- The **residual** of a word function after reading `a`: `(φ.residual a) x = φ (a :: x)` with its
    first letter dropped.  On a genuine Mealy behaviour this is again a behaviour. -/
def residual (φ : Word A → Word B) (a : A) : Word A → Word B := fun x => (φ (a :: x)).tail

/-- The **head output** of a word function on `a`: the first letter of `φ [a]` (with a default for
    the degenerate empty case). -/
def headOut [Inhabited B] (φ : Word A → Word B) (a : A) : B := (φ [a]).headD default

/-- On a Mealy behaviour, `residual` matches the underlying transition. -/
theorem residual_beh (M : Mealy A B σ) (s : σ) (a : A) :
    residual (beh M s) a = beh M (M.tr s a) := by
  apply funext; intro x
  show (beh M s (a :: x)).tail = beh M (M.tr s a) x
  have : beh M s (a :: x) = M.out s a :: beh M (M.tr s a) x := rfl
  rw [this]; rfl

/-- On a Mealy behaviour, `headOut` matches the underlying output function. -/
theorem headOut_beh [Inhabited B] (M : Mealy A B σ) (s : σ) (a : A) :
    headOut (beh M s) a = M.out s a := by
  show (beh M s [a]).headD default = M.out s a
  have : beh M s [a] = M.out s a :: beh M (M.tr s a) [] := rfl
  rw [this]; rfl

/-- The **minimal (Nerode) machine** of `M`: state space is the function space `Word A → Word B`,
    start is `M`'s behaviour, and the dynamics are defined intrinsically on word functions via
    `headOut`/`residual`, with no reference to `M`'s states.  Behaviourally-equal states collapse. -/
def minimal [Inhabited B] (M : Mealy A B σ) : Mealy A B (Word A → Word B) where
  start := beh M M.start
  out := fun φ a => headOut φ a
  tr := fun φ a => residual φ a

/-- **Running the minimal machine from a behaviour reproduces that behaviour.** -/
theorem minimal_run_beh [Inhabited B] (M : Mealy A B σ) (s : σ) (x : Word A) :
    ((minimal M).run (beh M s) x).1 = beh M s x := by
  induction x generalizing s with
  | nil => rfl
  | cons a x ih =>
      have e1 : ((minimal M).run (beh M s) (a :: x)).1
              = (minimal M).out (beh M s) a :: ((minimal M).run ((minimal M).tr (beh M s) a) x).1 := rfl
      rw [e1]
      show headOut (beh M s) a :: ((minimal M).run (residual (beh M s) a) x).1 = beh M s (a :: x)
      rw [headOut_beh, residual_beh, ih (M.tr s a)]; rfl

/-- **The minimal machine realizes the same relation as `M`.** -/
theorem minimal_realizes [Inhabited B] (M : Mealy A B σ) (x : Word A) (y : Word B) :
    (minimal M).realizes x y ↔ M.realizes x y := by
  show ((minimal M).run (beh M M.start) x).1 = y ↔ (M.run M.start x).1 = y
  rw [minimal_run_beh]; rfl

/-- In the minimal machine, every reachable state is its own behaviour. -/
theorem beh_minimal [Inhabited B] (M : Mealy A B σ) (s : σ) :
    beh (minimal M) (beh M s) = beh M s := by
  apply funext; intro x; exact minimal_run_beh M s x

/-- **Minimality / reducedness.**  Two reachable states of the minimal machine are behaviourally
    equivalent iff they are the same state -- the machine has no redundancy. -/
theorem minimal_reduced [Inhabited B] (M : Mealy A B σ) (s t : σ) :
    StateEquiv (minimal M) (beh M s) (beh M t) ↔ beh M s = beh M t := by
  constructor
  · intro h
    apply funext; intro x
    have hx := h x
    rw [minimal_run_beh, minimal_run_beh] at hx
    exact hx
  · intro h x; rw [minimal_run_beh, minimal_run_beh, h]

/-- **The relation realized by a Mealy machine is rational.**  Its witness is the transducer
    embedding `M.toTransducer`, with `toTransducer_realizes` providing the relational equivalence.
    This formally places the letter-to-letter class inside the rational relations, so the general
    closure theorems (`rational_union`, `rational_compose`, …) apply to Mealy relations. -/
theorem realizes_rational (M : Mealy A B σ) : Rational (fun x y => M.realizes x y) :=
  ⟨σ, M.toTransducer, fun x y => (toTransducer_realizes M x y).symm⟩

/-- **The Mealy relation is functional** (relation-level restatement of `realizes_functional`). -/
theorem realizes_Functional (M : Mealy A B σ) : Functional (fun x y => M.realizes x y) :=
  fun x y y' h h' => realizes_functional M x y y' h h'
end Mealy

/-- The **identity as a Mealy machine** over `A`: copies each letter, single state. -/
def idMealy (A : Type) : Mealy A A Unit where
  start := (); out := fun _ a => a; tr := fun _ _ => ()

private theorem idMealy_run (s : Unit) (x : Word A) : ((idMealy A).run s x).1 = x := by
  induction x generalizing s with
  | nil => rfl
  | cons a x ih =>
      have : ((idMealy A).run s (a :: x)).1 = a :: ((idMealy A).run () x).1 := rfl
      rw [this, ih ()]

/-- `idMealy` realizes the diagonal, mirroring `idT_realizes` for the Mealy class. -/
theorem idMealy_realizes (x y : Word A) : (idMealy A).realizes x y ↔ x = y := by
  unfold Mealy.realizes; rw [idMealy_run]

/-- **The identity is a left unit for Mealy composition:** `idMealy ⨾ M` realizes the same
    relation as `M`. -/
theorem comp_idMealy_left (M : Mealy A B σ) (x : Word A) (y : Word B) :
    (Mealy.comp (idMealy A) M).realizes x y ↔ M.realizes x y := by
  rw [Mealy.comp_realizes]
  constructor
  · rintro ⟨z, hz, hzy⟩; rw [idMealy_realizes] at hz; subst hz; exact hzy
  · intro h; exact ⟨x, (idMealy_realizes x x).mpr rfl, h⟩

/-- **The identity is a right unit for Mealy composition:** `M ⨾ idMealy` realizes the same
    relation as `M`.  With `comp_idMealy_left` and `Mealy.comp_realizes`, Mealy machines form a
    category: objects are alphabets, morphisms are Mealy transductions, composition is `Mealy.comp`,
    and `idMealy` is the identity. -/
theorem comp_idMealy_right (M : Mealy A B σ) (x : Word A) (y : Word B) :
    (Mealy.comp M (idMealy B)).realizes x y ↔ M.realizes x y := by
  rw [Mealy.comp_realizes]
  constructor
  · rintro ⟨z, hz, hzy⟩; rw [idMealy_realizes] at hzy; rw [← hzy]; exact hz
  · intro h; exact ⟨y, h, (idMealy_realizes y y).mpr rfl⟩



/-! ## The Mealy subclass as a predicate on transducers (`IsMealy`)

    EN: The `Mealy` structure above is a *model* of the letter-to-letter subclass, developed on its
        own total `run` function for clean proofs.  This section gives the intrinsic counterpart:
        `IsMealy`, a **predicate on general `Transducer`s** characterizing exactly the letter-to-
        letter, deterministic, total, single-start FSTs -- so a Mealy machine is literally an FST
        satisfying a property, with no separate type.

        The point of the predicate is that it *transports the special theorems directly onto the
        general model*.  `IsMealy.realizes_length` and `IsMealy.realizes_functional` are proved on
        the relational `PathN`, not via the structure -- and they are properties that are FALSE for
        general FSTs: an arbitrary transducer realizes a relation (not a function) and, through
        ε-moves, need not preserve length.  The bridge `Mealy.toTransducer_isMealy` shows the
        structure-based machines land in this predicate, so the two views agree.

        A note on the *converse* (`IsMealy T → ∃ a `Mealy` structure realizing the same relation`):
        it is **not provable within this library's axiom budget**, and the reason is mathematical,
        not an omission.  Extracting the output and transition *functions* `out : σ → A → B` and
        `tr : σ → A → σ` from the existential `IsMealy.total` (`∀ p a, ∃ b q, …`) is exactly an
        application of the axiom of choice; with `DecidableEq` and an enumeration of states it could
        be done by constructive search, but in general it requires `Classical.choice`, which the
        library forbids.  So the formalized relationship is: the `Mealy` structure is a *model* of
        `IsMealy` (`toTransducer_isMealy`), and `IsMealy` transports the sequential-function
        theorems below onto the general model -- the inclusion and the property-transfer are proved;
        the choice-dependent converse is deliberately not. -/

/-- A general transducer is **Mealy** when every transition reads exactly one input letter and
    writes exactly one output letter (no ε-moves), from each state every input letter has a unique
    output-and-successor (deterministic + total), and there is a single start state.  This is the
    subclass of FSTs whose realized relation is a length-preserving sequential *function*. -/
structure IsMealy (T : Transducer A B σ) : Prop where
  /-- Every step consumes one input letter and emits one output letter (no ε-moves). -/
  letterToLetter : ∀ p oa ob q, T.step p oa ob q → ∃ a b, oa = some a ∧ ob = some b
  /-- From each state, every input letter has at least one (output, successor). -/
  total : ∀ p a, ∃ b q, T.step p (some a) (some b) q
  /-- That (output, successor) is unique: deterministic. -/
  deterministic : ∀ p a b₁ q₁ b₂ q₂,
    T.step p (some a) (some b₁) q₁ → T.step p (some a) (some b₂) q₂ → b₁ = b₂ ∧ q₁ = q₂
  /-- A single start state. -/
  startUnique : ∀ p p', T.start p → T.start p' → p = p'

/-- **Length preservation transports to any Mealy FST** (proved on the general `PathN`, using only
    that each step contributes one input and one output letter).  False for general FSTs. -/
theorem IsMealy.path_length {T : Transducer A B σ} (hM : IsMealy T)
    (n : Nat) (p q : σ) (x : Word A) (y : Word B) :
    PathN T n p x y q → x.length = y.length := by
  induction n generalizing p x y with
  | zero => intro h; obtain ⟨_, hx, hy⟩ := PathN.zero_inv h; rw [hx, hy]; rfl
  | succ m ih =>
      intro h
      obtain ⟨oa, ob, r, x', y', hstep, htail, hx, hy⟩ := PathN.step_inv h
      obtain ⟨a, b, hoa, hob⟩ := hM.letterToLetter _ _ _ _ hstep
      subst hoa; subst hob; subst hx; subst hy
      simp only [olist, List.singleton_append, List.length_cons]
      rw [ih r x' y' htail]

/-- **Length preservation at the relation level.** -/
theorem IsMealy.realizes_length {T : Transducer A B σ} (hM : IsMealy T)
    (x : Word A) (y : Word B) : realizes T x y → x.length = y.length := by
  rintro ⟨p, q, _, _, n, hpath⟩
  exact hM.path_length n p q x y hpath

/-- A Mealy path reading empty input is the trivial path: there are no ε-moves to consume input
    silently, so the step count is zero and nothing is emitted. -/
theorem IsMealy.empty_input {T : Transducer A B σ} (hM : IsMealy T)
    (n : Nat) (p q : σ) (y : Word B) : PathN T n p [] y q → n = 0 ∧ y = [] ∧ p = q := by
  cases n with
  | zero => intro h; obtain ⟨hpq, _, hy⟩ := PathN.zero_inv h; exact ⟨rfl, hy, hpq⟩
  | succ m =>
      intro h
      obtain ⟨oa, ob, r, x', y', hstep, htail, hx, hy⟩ := PathN.step_inv h
      obtain ⟨a, b, hoa, hob⟩ := hM.letterToLetter _ _ _ _ hstep
      subst hoa
      simp only [olist, List.singleton_append] at hx
      exact absurd hx (by simp)

/-- **Unique runs.**  Two Mealy runs from the same state on the same input agree on both output and
    end state -- determinism + totality + no-ε force the run to be unique.  This is the engine
    behind functionality. -/
theorem IsMealy.path_unique {T : Transducer A B σ} (hM : IsMealy T)
    (x : Word A) : ∀ (n₁ n₂ : Nat) (p q₁ q₂ : σ) (y₁ y₂ : Word B),
    PathN T n₁ p x y₁ q₁ → PathN T n₂ p x y₂ q₂ → y₁ = y₂ ∧ q₁ = q₂ := by
  induction x with
  | nil =>
      intro n₁ n₂ p q₁ q₂ y₁ y₂ h1 h2
      obtain ⟨_, hy1, hpq1⟩ := hM.empty_input n₁ p q₁ y₁ h1
      obtain ⟨_, hy2, hpq2⟩ := hM.empty_input n₂ p q₂ y₂ h2
      exact ⟨by rw [hy1, hy2], by rw [← hpq1, ← hpq2]⟩
  | cons a x' ih =>
      intro n₁ n₂ p q₁ q₂ y₁ y₂ h1 h2
      cases n₁ with
      | zero => obtain ⟨_, hx, _⟩ := PathN.zero_inv h1; exact absurd hx.symm (by simp)
      | succ m₁ =>
      cases n₂ with
      | zero => obtain ⟨_, hx, _⟩ := PathN.zero_inv h2; exact absurd hx.symm (by simp)
      | succ m₂ =>
        obtain ⟨oa1, ob1, r1, xa1, ya1, hs1, ht1, hx1, hy1⟩ := PathN.step_inv h1
        obtain ⟨oa2, ob2, r2, xa2, ya2, hs2, ht2, hx2, hy2⟩ := PathN.step_inv h2
        obtain ⟨c1, d1, hoa1, hob1⟩ := hM.letterToLetter _ _ _ _ hs1
        obtain ⟨c2, d2, hoa2, hob2⟩ := hM.letterToLetter _ _ _ _ hs2
        subst hoa1; subst hoa2
        simp only [olist, List.singleton_append] at hx1 hx2
        obtain ⟨rfl, rfl⟩ := List.cons.inj hx1
        obtain ⟨rfl, rfl⟩ := List.cons.inj hx2
        subst hob1; subst hob2
        obtain ⟨hd, hr⟩ := hM.deterministic p a d1 r1 d2 r2 hs1 hs2
        subst hd; subst hr
        obtain ⟨hyt, hqt⟩ := ih m₁ m₂ r1 q₁ q₂ ya1 ya2 ht1 ht2
        subst hy1; subst hy2
        exact ⟨by rw [hyt], hqt⟩

/-- **Functionality transports to any Mealy FST.**  The realized relation of an `IsMealy`
    transducer is a partial function: each input has at most one output.  This is FALSE for general
    FSTs (which realize arbitrary relations); it holds here because the unique start state and
    `path_unique` force a single run per input. -/
theorem IsMealy.realizes_functional {T : Transducer A B σ} (hM : IsMealy T)
    (x : Word A) (y₁ y₂ : Word B) : realizes T x y₁ → realizes T x y₂ → y₁ = y₂ := by
  rintro ⟨p₁, q₁, hsp1, _, n₁, hpath1⟩ ⟨p₂, q₂, hsp2, _, n₂, hpath2⟩
  obtain rfl : p₁ = p₂ := hM.startUnique p₁ p₂ hsp1 hsp2
  exact (hM.path_unique x n₁ n₂ p₁ q₁ q₂ y₁ y₂ hpath1 hpath2).1


/-- **Step count equals input length** for a Mealy run: each step consumes exactly one input
    letter.  This pins runs to input prefixes, enabling the causality split below. -/
theorem IsMealy.steps_eq_len {T : Transducer A B σ} (hM : IsMealy T)
    (n : Nat) (p q : σ) (x : Word A) (y : Word B) : PathN T n p x y q → n = x.length := by
  induction n generalizing p x y with
  | zero => intro h; obtain ⟨_, hx, _⟩ := PathN.zero_inv h; rw [hx]; rfl
  | succ m ih =>
      intro h
      obtain ⟨oa, ob, r, x', y', hstep, htail, hx, hy⟩ := PathN.step_inv h
      obtain ⟨a, b, hoa, hob⟩ := hM.letterToLetter _ _ _ _ hstep
...
/-- **Causality / online property transports to any Mealy FST.**  If `T` realizes both `(x, y)` and
    `(x ++ x', w)`, then `y <+: w`.  A Mealy FST commits to each output letter as it reads, so
    extending the input only *extends* the output -- the defining property of a sequential function.
    This is FALSE even for *functional* general FSTs, which may delay output and revise it in light
    of later input.  Proof: split the longer run at the input boundary `|x|`; its prefix is a run on
    `x`, whose output equals `y` by `path_unique`, so `w = y ++ (suffix output)`. -/
theorem IsMealy.realizes_prefix {T : Transducer A B σ} (hM : IsMealy T)
    (x x' : Word A) (y w : Word B)
    (hy : realizes T x y) (hw : realizes T (x ++ x') w) : y <+: w := by
  obtain ⟨py, qy, hspy, _, ny, hpy⟩ := hy
  obtain ⟨pw, qw, hspw, _, nw, hpw⟩ := hw
  obtain rfl : py = pw := hM.startUnique py pw hspy hspw
  have hnw : nw = (x ++ x').length := IsMealy.steps_eq_len hM nw py qw (x ++ x') w hpw
  have hle : x.length ≤ nw := by rw [hnw, List.length_append]; omega
  obtain ⟨r, x1, y1, x2, y2, hpre, hsuf, hxeq, hyeq⟩ := PathN.split hpw x.length hle
  have hx1len : x.length = x1.length := IsMealy.steps_eq_len hM x.length py r x1 y1 hpre
  have hx1 : x1 = x := by
    have h1 : (x ++ x').take x1.length = x1 := by rw [hxeq]; exact List.take_left
    rw [← hx1len] at h1
    rw [List.take_append_of_le_length (Nat.le_refl x.length), List.take_length] at h1
    exact h1.symm
  rw [hx1] at hpre
  have hy1 : y1 = y := (hM.path_unique x x.length ny py r qy y1 y hpre hpy).1
  rw [hy1] at hyeq
  exact ⟨y2, hyeq.symm⟩
/-- **The Mealy embedding lands in the `IsMealy` subclass.**  Every transducer of the form
    `M.toTransducer` satisfies `IsMealy`, so the structure-based Mealy machines are exactly a model
    of the predicate-defined letter-to-letter FSTs -- the two formulations agree. -/
theorem Mealy.toTransducer_isMealy (M : Mealy A B σ) : IsMealy M.toTransducer := by
  constructor
  case letterToLetter =>
    intro p oa ob q h; obtain ⟨a, hoa, hob, _⟩ := h; exact ⟨a, M.out p a, hoa, hob⟩
  case total =>
    intro p a; exact ⟨M.out p a, M.tr p a, ⟨a, rfl, rfl, rfl⟩⟩
  case deterministic =>
    intro p a b₁ q₁ b₂ q₂ h1 h2
    obtain ⟨a₁, ha₁, hb₁, hq₁⟩ := h1
    obtain ⟨a₂, ha₂, hb₂, hq₂⟩ := h2
    have haa : a₁ = a₂ := Option.some.inj (ha₁.symm.trans ha₂)
    subst haa
    have hb : b₁ = b₂ := Option.some.inj (hb₁.trans hb₂.symm)
    exact ⟨hb, hq₁.trans hq₂.symm⟩
  case startUnique =>
    intro p p' hp hp'; rw [hp, hp']
